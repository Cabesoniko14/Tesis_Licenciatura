    ...
    def get_llm_evaluation(self, prev_state, action, next_state, agent_letter):
        try:
            response = self.client.chat.completions.create(
                messages=[{
                    "role": "user",
                    "content": f"Eres un evaluador de un agente de aprendizaje por refuerzo. Específicamente, funges como función de recompensas de un agente que está aprendiendo a jugar tic tac toe/gato. Debes ser ni muy poco informativo al presente para que no se estanque, ni demasiado informativo para trivializar. Se debe valorar bloquear jugadas y progresar a buenas jugadas. Las victorias y no perder son lo más importante. No te estanques en dar solo recompensas regulares. Da menor valor a tomar demasiado tiempo en tomar acciones óptimas, o tomar acciones que no hagan sentido.\n"
                               f"El tablero antes de la acción: {prev_state}\n"
                               f"El agente ({agent_letter}) coloca en la posición {action}\n"
                               f"El tablero resultante: {next_state}\n"
                               f"Evalúa esta acción y responde solo y estrictamente con: SUPER BAD, BAD, REGULAR, GOOD, SUPER GOOD."
                               f"Tu respuesta de texto SOLO puede ser una de las palabras mencionadas: SUPER BAD, BAD, REGULAR, GOOD, SUPER GOOD. Si no, tronará el modelo. Solo debes responder una de las opciones."
                               f"No agregues explicaciones ni nada por el estilo. Solo una de las opciones: SUPER BAD, BAD, REGULAR, GOOD, SUPER GOOD"
                }],
...
def train_dqn_agent_vs_random_llm(num_epochs=5, episodes_per_epoch=10):
    ...
    base = f"tictactoe_deepqn_groq_random_{timestamp}_episodes_{total_episodes}"
