# ======================  DQN: FrozenLake (Gymnasium) — con Global Average Pooling (GAP)  ======================
import os
import time
import json
import psutil
...
class QNetCNN_GAP(nn.Module):
    ...
    Funciona para cualquier HxW (4x4, 5x5, 8x8, etc.)
    """
...
def train_dqn_frozenlake(
    ...
    # Directorios y base (incluye "gap")
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    total_episodes = num_epochs * episodes_per_epoch
    base = (
        f"frozenlake_dqn_gap_{map_mode}_size{map_size}_"
        f"{'slip' if is_slippery else 'noslip'}_{timestamp}_episodes_{total_episodes}"
    )
    ...
    # Redes + buffer + optim
    policy = QNetCNN_GAP(in_channels=4, n_actions=n_actions).to(device)
    target = QNetCNN_GAP(in_channels=4, n_actions=n_actions).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for ep in range(episodes_per_epoch):
            ...
            # fija el desc del episodio (evita confusiones)
            desc_ep = env.unwrapped.desc  # bytes (H,W)

            obs_tensor = encode_obs_from_desc(desc_ep, s_idx)  # (C,H,W)
            done = False
            reward_acc = 0.0
            ...
            for t in range(max_steps_per_ep):
                ...
                next_obs_tensor = encode_obs_from_desc(desc_ep, ns_idx)
    ...
    resumen_df.loc[len(resumen_df)] = ["Modo", f"{map_mode} | is_slippery={is_slippery} | size={map_size} | net=gap"]
...
def evaluate_agent_dqn(
    ...
    # Carga red GAP
    policy = QNetCNN_GAP(in_channels=4, n_actions=4).to(device)
    ...
    for _ in range(episodes):
        ...
        obs, _ = env.reset()
        s_idx = int(obs)
        desc_ep = env.unwrapped.desc

        for _ in range(max_steps):
            x = torch.from_numpy(encode_obs_from_desc(desc_ep, s_idx)).unsqueeze(0).to(device)
