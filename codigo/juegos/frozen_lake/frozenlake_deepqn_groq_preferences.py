...
# Cliente LLM (toma la API key tal cual lo haces)
...
def _grid_to_str(desc):
    ...
    idx_map = []
    for r in range(H):
        idx_row = [f"{r*W + c:02d}" for c in range(W)]
        idx_map.append(" ".join(idx_row))

    return "\n".join(rows) + "\n\nIndex map (row-major):\n" + "\n".join(idx_map)

def _tile_at(desc, idx):
    H, W = desc.shape
    r, c = idx_to_rc(idx, W)
    v = desc[r, c]
    if isinstance(v, (bytes, np.bytes_)):
...
def _format_path(desc, path_states):
    """
    path_states: lista de índices [s0, s1, ..., sT]
    Retorna string con idx->(r,c)->tile
    """
    H, W = desc.shape
    parts = []
    for i, s in enumerate(path_states):
        r, c = idx_to_rc(int(s), W)
        tile = _tile_at(desc, int(s))
        parts.append(f"{i:02d}: idx={int(s):02d} -> (r={r},c={c}) tile='{tile}'")
    return "\n".join(parts)
...
def _format_actions(actions):
    ...
    for i, a in enumerate(actions):
        parts.append(f"{i:02d}: a={int(a)} ({action_names.get(int(a), str(a))})")
    return "\n".join(parts)

def _termination_reason(terminated, truncated, final_tile, env_reward_last):
    """
    ...
    if bool(terminated) and float(env_reward_last) == 1.0 and final_tile == "G":
        return "TERMINATED (llegó a la META 'G')"
    if bool(terminated) and final_tile == "H":
        return "TERMINATED (cayó en un HOYO 'H')"
    if bool(terminated):
        return "TERMINATED (fin del episodio; no meta)"
    return "NOT_DONE"
...
def reward_proxy_episode_llm(
    *,
    ...
    """
    - Si NO terminó: 0.0
    - Si terminó: consulta LLM con grid + path + acciones + causa + directiva y devuelve val en [0,1]
      Si falla, hace fallback al expected de gym: 1 si env_reward_last==1 y terminated else 0
    """
    ...
    H, W = desc.shape
    grid_str = _grid_to_str(desc)

    final_state = int(path_states[-1]) if len(path_states) else None
    final_tile = _tile_at(desc, final_state) if final_state is not None else "?"
    reason = _termination_reason(terminated, truncated, final_tile, env_reward_last)
    ...
    prompt = (
        ...
        "REGLAS del entorno:\n"
        "- Mapa con celdas: 'S' inicio, 'F' seguro, 'H' hoyo, 'G' meta.\n"
        "- Acciones: 0=Left, 1=Down, 2=Right, 3=Up.\n"
        "- Termina y ganasi llega a 'G', termina y pierde si cae en 'H', o pierde y se trunca por límite de pasos.\n\n"
        "DIRECTIVA del usuario (preferencias de entrenamiento):\n"
        f"{directive}\n\n"
        ...
        f"- final_state_idx={final_state}\n"
        f"- final_tile='{final_tile}'\n"
        f"- termination_reason: {reason}\n\n"
        "MAPA (por filas) + mapa de índices row-major:\n"
        f"{grid_str}\n\n"
        "CAMINO (estados visitados en orden):\n"
        f"{path_str}\n\n"
        "ACCIONES (en orden):\n"
        f"{actions_str}\n\n"
        "INSTRUCCIÓN FINAL:\n"
        "- Devuelve un número en [0,1] sin más texto, solo el número, de acuerdo a la directiva.\n"
    )
    ...
    try:
        ...
        val = _parse_first_float(raw, lo=lo, hi=hi)
...
def train_dqn_frozenlake(
    ...
    base = f"frozenlake_dqn_llm_episodic_preferences_2_safest_route_{map_mode}_size{map_size}_{'slip' if is_slippery else 'noslip'}_{timestamp}_episodes_{total_episodes}"
    ...
    for epoch in range(num_epochs):
        ...
        for ep in range(episodes_per_epoch):
            ...
            for t in range(max_steps_per_ep):
                ...
                episode_states.append(ns_idx)

                # obs siguiente
                next_desc = desc if map_mode == "fixed" else env.unwrapped.desc
    ...
    resumen_df.loc[len(resumen_df)] = ["use_llm_reward", bool(use_llm_reward)]
    resumen_df.loc[len(resumen_df)] = ["directive", directive]
    resumen_df.loc[len(resumen_df)] = ["llm_model", llm_model]
    resumen_df.loc[len(resumen_df)] = ["llm_temperature", llm_temperature]
    ...
    meta = {
        ...
        "use_llm_reward": bool(use_llm_reward),
        "directive": directive,
        "llm_model": llm_model,
