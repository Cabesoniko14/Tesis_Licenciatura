...
def _grid_to_str(desc):
    ...
    for r in range(H):
        ...
        for c in range(W):
            v = desc[r, c]
            # Gymnasium devuelve np.bytes_; decodificamos a str
    ...
    # Mapa de índices row-major para referencia
    idx_map = []
    for r in range(H):
...
def reward_proxy(terminated, truncated, env_reward_last, desc, last_state_idx, last_action, next_state_idx):
    ...
    # Regla "ground truth" del entorno (para fallback y warnings)
    expected = 1.0 if (bool(terminated) and float(env_reward_last) == 1.0) else 0.0

    # Info del grid / indices
    H, W = desc.shape
    grid_str = _grid_to_str(desc)

    # Acción en FrozenLake: 0=Left, 1=Down, 2=Right, 3=Up
    ...
    try:
        ...
        # Si el expected es 0 o 1 (lo es en FrozenLake), advertimos si no coincide
        ...
        # Usamos la recompensa del LLM (como pediste)
        return float(val)
